...
/**
  ...
  which changes int& to int, basically reference to value
  conversion.
 */

template <unsigned int... Vals> struct UintContainer;
template <unsigned long... Vals>
struct UnsignedLongContainer {
  constexpr std::array<unsigned long, sizeof...(Vals)>
  operator()() {
    std::array<unsigned long, sizeof...(Vals)> arr = {
        Vals...};
    return arr;
  }
};
...
constexpr std::pair<bool, unsigned int>
find_nontype(const unsigned int &&e) {
  //
...
find_nontype(const unsigned int &&e) {
  if
    constexpr(El == e) { return std::make_pair(true, El); }
...
// base case: char
template <char... Nums>
constexpr std::pair<bool, char>
find_nontype(const char &&e) {
  //
  return std::make_pair(false, e);
}
// recursive case
...
find_nontype(const bool &&e) {
  if
    constexpr(e == El) { return std::make_pair(true, El); }
  return find_nontype<Nums...>(e);
}
// base case: size_t
template <std::size_t... Nums>
constexpr std::pair<bool, std::size_t>
find_nontype(const std::size_t &&e) {
  //
  return std::make_pair(false, e);
}
// recursive case
...
constexpr T binop = [](T Acc, T arg) -> T {
  if
    constexpr(op == ArtOp::PLUS) { return Acc + arg; }
  if
    constexpr(op == ArtOp::MINUS) { return Acc - arg; }
  if
    constexpr(op == ArtOp::MULTIPLY) { return Acc * arg; }
};
// base case: uint
template <ArtOp op, unsigned int... Nums>
constexpr unsigned int uint_foldl() {
  if
    constexpr(op == ArtOp::PLUS) return 0;
  if
    constexpr(op == ArtOp::MINUS) return 0;
  if
    constexpr(op == ArtOp::MULTIPLY) return 1;
  return 0;
}
// recursive case
template <ArtOp op, unsigned int Acc, unsigned int... Nums>
constexpr unsigned int uint_foldl() {
  return binop<unsigned int, op>(Acc,
                                 uint_foldl<op, Nums...>);
}
// base case: int
...
// recursive case
template <ArtOp op, int Acc, int... Nums>
constexpr int int_foldl() {
  return binop<int, op>(Acc, int_foldl<op, Nums...>);
}
// base case: std::size_t
template <ArtOp op, std::size_t... Nums>
constexpr std::size_t size_foldl() {
  if
    constexpr(
        op ==
        ArtOp::PLUS) return static_cast<std::size_t>(0);
  if
    constexpr(
        op ==
        ArtOp::MINUS) return static_cast<std::size_t>(0);
  if
    constexpr(
        op ==
        ArtOp::MULTIPLY) return static_cast<std::size_t>(1);
  return static_cast<std::size_t>(0);
}
// recursive case
template <ArtOp op, std::size_t Acc, std::size_t... Nums>
constexpr std::size_t size_foldl() {
  return binop<std::size_t, op>(Acc,
                                size_foldl<op, Nums...>);
}
...
struct SingleTemplateCont;
template <template <typename...> class... T>
struct MultiTemplateCont;

template <typename Return, typename OverloadFn,
          typename... Args>
struct op {
  Return operator()(const OverloadFn &f, Args... args) {
    return f(args...);
  }
};
...
// 1 - N
template <typename OverloadFn, typename InArg,
          typename... OutArgs>
struct expander {
  std::tuple<OutArgs...> operator()(const OverloadFn &f,
                                    const InArg &inArg) {
    return f(inArg);
  }
};
...
template <typename Fn, typename OutArg, typename... InArgs>
struct reducer {
...
// N - N
template <std::size_t... TupleIndices, typename RetTuple,
          typename OverloadedFn, typename Tuple1>
constexpr RetTuple
mapperHelper(std::index_sequence<TupleIndices...>,
             Tuple1 t1, const OverloadedFn &f) {
  return std::make_tuple(f(std::get<TupleIndices>(t1))...);
}

template <typename OverloadedFn, typename OutArgsList,
          typename InArgsList>
struct mapper {};

template <typename OverloadedFn, typename... OutArgs,
          typename... InArgs>
struct mapper<OverloadedFn, TypeList<OutArgs...>,
              TypeList<InArgs...>> {
  std::tuple<OutArgs...> operator()(const OverloadedFn &f,
                                    InArgs... inArgs) {
...
template <typename OverloadedFn, typename TypeLOut,
          typename TypeLIn>
struct TupleCall {};

template <typename OverloadedFn, typename... OutArgs,
          typename... InArgs>
struct TupleCall<OverloadedFn, TypeList<OutArgs...>,
                 TypeList<InArgs...>> {
  constexpr std::tuple<OutArgs...>
  operator()(const OverloadedFn &f, InArgs... inArgs) {
    constexpr auto intpl = std::make_tuple(inArgs...);
    return f(intpl);
  }
};
...
template <std::size_t Min, std::size_t Max, typename Acc,
          typename El, typename... Els>
struct foldl {
  constexpr void
  operator()(const std::function<Acc(Acc, El)> &f, Acc &a,
             Els... els) {
    if
      constexpr(Min < Max) {
...
constexpr void foldfn(const Fn &f, Acc &a, Els... els) {
  ...
  foldl<0, sizeof...(Els), Acc, decltype(std::get<0>(es)),
        Els...>(f, a, els...);
}
...
template <typename OverloadedFn, typename RetList,
          typename ArgList1, typename ArgList2>
struct merger {};

template <typename OverloadedFn, typename... RetVals,
          typename... Args1, typename... Args2>
struct merger<OverloadedFn, TypeList<RetVals...>,
              TypeList<Args1...>, TypeList<Args2...>> {

  std::tuple<RetVals...> operator()(const OverloadedFn &fn,
                                    Args1... args1,
                                    Args2... args2) {
    //
...
template <class... TupleType, std::size_t N,
          std::size_t... Indice>
constexpr std::tuple<TupleType...>
cast_to_tuple(std::array<unsigned int, N> const &arr,
              std::index_sequence<Indice...>) {
  //
  return std::make_tuple(
      static_cast<TupleType>(arr[Indice])...);
}

template <
    class... TupleType, std::size_t N,
    class = std::enable_if_t<(N == sizeof...(TupleType))>>
constexpr std::tuple<TupleType...>
cast_to_tuple(std::array<unsigned int, N> const &arr) {
  //
  return cast_to_tuple<TupleType...>(
